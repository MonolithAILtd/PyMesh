...
def extract_rpaths(lib_file):
    ...
    if system_name == "linux":
        ...
        raise NotImplementedError("Platform {} is not supported.".format(system_name))
...
def migrate_dependency_darwin(lib_file, dep_file, dep_dir, rpaths):
    ...
    cmd = "install_name_tool -change {} {} {}".format(
        dep_file, "@rpath/{}".format(dep_lib_name), lib_file
    )
    check_call(cmd.split())
...
def reset_rpath_posix(lib_file, dep_dir, rpaths):
    ...
    cmd = "patchelf --set-rpath $ORIGIN:$ORIGIN/{} {}".format(rel_dep_dir, lib_file)
    check_call(cmd.split())
