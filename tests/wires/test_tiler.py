...
from .base_wire_case import WireTestCase
...
class TilerTest(WireTestCase):
    def test_tile_with_bbox(self):
        ...
        self.assertEqual(8 * wire_network.num_edges, tiled_wire_network.num_edges)
        self.assertEqual(
    ...
    def test_tile_with_guide_mesh(self):
        ...
        mesh = generate_box_mesh(
            np.zeros(3), np.ones(3), subdiv_order=1, using_simplex=False
        )
    ...
    def test_tile_with_mixed_patterns(self):
        wire_networks = [self.get_brick5(), self.get_cross_3D()]
        params = [Parameters(wire_networks[0], 0.1), Parameters(wire_networks[1], 0.0)]
        ...
        for i in range(max_num_dofs):
            ...
            dof = np.array(
                [params[j].dofs[i] if i < params[j].num_dofs else 0 for j in pattern_id]
            )
        ...
        self.assertEqual(
            (wire_networks[0].num_vertices + wire_networks[1].num_vertices) * 4 - 4 * 3,
