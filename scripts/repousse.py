...
from pymesh.snap_rounding import snap_rounding
from pymesh.triangle import triangle
from pymesh import igl_utils
...
def parse_args():
    ...
    parser.add_argument(
        "--bevel", type=float, default=0.0, help="Bevel edges by this amount"
    )
    parser.add_argument(
        "--extrude", type=float, default=0.0, help="Extrude input graphics"
    )
...
def cleanup(wires, logger):
    ...
    start_time = time()
    tol = 1e-6
    vertices, edges, __ = mu.remove_duplicated_vertices_raw(
        wires.vertices, wires.edges, tol
    )

    # Remove duplicated edges.
    ordered_edges = np.sort(edges, axis=1)
    __, unique_edge_ids, __ = igl_utils.unique_rows(ordered_edges)
...
def uniform_sampling(wires):
    ...
    for i in range(wires.num_edges):
        ...
        new_edges = [
            [vertex_count + j - 1, vertex_count + j] for j in range(int(num_samples))
        ]
...
def resolve_self_intersection(wires, logger):
    ...
    tol = norm(bbox_max - bbox_min) / 1000
    start_time = time()
    vertices, edges = snap_rounding(wires.vertices, wires.edges, tol)
    finish_time = time()
    t = finish_time - start_time
...
def extract_straight_skeleton(wires, logger, remove_holes):
    ...
    mesh = mu.subdivide(mesh, order=1, method="simple")
...
def bevel(wires, logger, remove_holes, dist):
    ...
    for i in range(num_cells):
        ...
        cut_mesh = submesh(mesh, to_keep, 0)
        bd_edges = cut_mesh.boundary_edges
        vertices, edges, __ = mu.remove_isolated_vertices_raw(
            cut_mesh.vertices, bd_edges
        )
        bd_wires = wires.WireNetwork.create_from_data(vertices, edges)
        ...
        comp_wires = wires.WireNetwork.create_from_data(vertices, edges)
        comp = constrained_triangulate(comp_wires, logger, True)
        comps.append(comp)

    mesh = mu.merge_meshes(comps)
    bd_vertices = mesh.boundary_vertices
    ...
    mesh = mio.form_mesh(vertices, mesh.faces)
    return mesh
...
def repousse_all(mesh, logger):
    ...
    out_mesh, info = mu.remove_degenerated_triangles(mesh, 100)
    cell_ids = cell_ids[info["ori_face_indices"]].ravel()
    mesh, info = mu.split_long_edges(out_mesh, tol)
    cell_ids = cell_ids[info["ori_face_indices"]].ravel()
    ...
    solver = SparseSolver.create("SparseLU")
    solver.compute(A)
    x = solver.solve(rhs)
    z = x[:dof].reshape((-1, 1))
    ...
    return mio.form_mesh(vertices, mesh.faces)
...
def repousse(mesh, logger):
    ...
    for i in range(num_cells):
        ...
        cut_mesh = submesh(mesh, to_keep, 0)
        mio.save_mesh("debug.msh", cut_mesh)
        cut_mesh, __ = mu.remove_degenerated_triangles(cut_mesh, 100)
        cut_mesh, __ = mu.split_long_edges(cut_mesh, tol)

        dof = cut_mesh.num_vertices
        assembler = Assembler(cut_mesh)
        L = assembler.assemble("laplacian")
        M = assembler.assemble("mass")
        ...
        out_mesh = mio.form_mesh(vertices, cut_mesh.faces)
    ...
    mesh = mu.merge_meshes(results)

    vertices = mesh.vertices[:, :2]
    mesh_2d = mio.form_mesh(vertices, mesh.faces)
    mio.save_mesh("out_2d.msh", mesh_2d)
    return mesh
...
def extrude(mesh, depth, logger):
    ...
    return mio.form_mesh(vertices, faces)


def main():
    args = parse_args()
    logger = get_logger(args.log)
    wires = wires.WireNetwork.create_from_file(args.input_svg)
