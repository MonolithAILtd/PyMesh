...
def tile_with_guide_box(config):
    ...
    inflator.inflate(
        network.get_attribute("thickness").ravel(), parameters.per_vertex_thickness
    )
...
def tile_with_guide_mesh(config):
    ...
    network = load_wire(str(config["wire_network"]))
    parameters = load_parameters(network, config)
    guide_mesh = mio.load_mesh(config["guide_mesh"])
...
def tile_with_mixed_patterns(config):
    ...
    mesh = mio.load_mesh(config["guide_mesh"])
    per_vertex_thickness = options["thickness_type"] == "vertex"

    tiler = Tiler()
    tiler.tile_with_mixed_patterns(
        mesh, per_vertex_thickness, options["dof_type"] == "isotropic"
    )
    network = tiler.wire_network
    ...
    inflator.subdivide_order = options["subdiv"]
    inflator.subdivide_method = options["subdiv_method"]
    inflator.inflate(network.get_attribute("thickness").ravel(), per_vertex_thickness)
    return inflator.mesh
...
def main():
    ...
    mesh = tile(config)
    mio.save_mesh(config["output"], mesh, *mesh.get_attribute_names())
