...
def parse_args():
    ...
    parser.add_argument(
        "--charge-distribution", "-c", choices=["sphere", "center"], default="sphere"
    )
...
def test_function(charges):
    ...
    """
    return lambda pts: np.sum(
        [1.0 / numpy.linalg.norm(pts - c, axis=1) for c in charges], axis=0
    )
...
def test_function_grad(charges):
    ...
    return lambda pts: np.sum(
        [
            -(pts - c) / np.power(numpy.linalg.norm(pts - c, axis=1), 3)[:, np.newaxis]
            for c in charges
        ],
        axis=0,
    )
...
def fit_into_unit_sphere(mesh):
    ...
    return mio.form_mesh(vertices, tris, tets)
...
def main():
    ...
    if args.charge_distribution == "sphere":
        ball = mu.generate_icosphere(1.25 + 1e-3, np.zeros(3), 0)
        charges = ball.vertices
    elif args.charge_distribution == "center":
        charges = np.zeros(3)
    else:
        raise NotImplementedError(
            "Unsupported charge distribution ({}).".format(args.charge_distribution)
        )

    if args.output_kernels:
        kernels = mu.merge_meshes([mu.generate_icosphere(0.05, v, 2) for v in charges])
        mio.save_mesh("kernel.msh", kernels)
    ...
    solver = HarmonicSolver.create(mesh)
    solver.order = 1
    ...
    sol_q_values, sol_q_grad = interpolate_at_quadrature_pts(mesh, sol_values, sol_grad)
    sol_c_values, sol_c_grad = interpolate_at_centroids(mesh, sol_values, sol_grad)
    ...
    print(
        "L2 grad solution at quadrature pts: {}".format(numpy.linalg.norm(q_grad_norm))
    )
    ...
    print(
        "sqrt ave at quadrature pts: {}".format(math.sqrt(np.mean(np.square(q_values))))
    )
    print(
        "sqrt ave grad solution at centroids: {}".format(
            math.sqrt(np.mean(np.square(c_grad_norm)))
        )
    )
    print(
        "sqrt ave grad solution at quadrature pts: {}".format(
            math.sqrt(np.mean(np.square(q_grad_norm)))
        )
    )
    ...
    print(
        "L2 grad error at quadrature pts: {}".format(numpy.linalg.norm(q_grad_err_norm))
    )
    ...
    print(
        "max grad rel error at quadrature pts: {}".format(np.amax(q_grad_rel_err_norm))
    )
    ...
    mesh.set_attribute(
        "q_error", np.amax(np.absolute(q_err.reshape((-1, 4), order="C")), axis=1)
    )
    mesh.add_attribute("q_rel_error")
    mesh.set_attribute(
        "q_rel_error",
        np.amax(np.absolute(q_rel_err.reshape((-1, 4), order="C")), axis=1),
    )
    ...
    mesh.set_attribute(
        "q_grad_error", np.amax(q_grad_err_norm.reshape((-1, 4), order="C"), axis=1)
    )
    mesh.add_attribute("q_grad_rel_error")
    mesh.set_attribute(
        "q_grad_rel_error",
        np.amax(q_grad_rel_err_norm.reshape((-1, 4), order="C"), axis=1),
    )
    ...
    mio.save_mesh(args.output_mesh, mesh, *mesh.attribute_names)
