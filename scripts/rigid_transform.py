...
def parse_args():
    ...
    parser.add_argument(
        "--offset", "-o", nargs=3, type=float, help="offset vector", default=[0, 0, 0]
    )
    parser.add_argument(
        "--rotation-axis", nargs=3, type=float, help="rotation axis", default=[1, 0, 0]
    )
    parser.add_argument(
        "--rotation-angle", type=float, default=0, help="rotation angle"
    )
...
def main():
    ...
    mio.save_mesh_raw(args.output_mesh, vertices, mesh.faces, mesh.voxels)
