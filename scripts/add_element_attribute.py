...
class Formula:
    ...
    def __load_default_fields(self):
        ...
        if self.mesh.num_voxels == 0:
            ...
            elem_centers = elem_centers.reshape((self.mesh.num_faces, self.mesh.dim))
        else:
            self.mesh.add_attribute("voxel_centroid")
            elem_centers = self.mesh.get_attribute("voxel_centroid")
            elem_centers = elem_centers.reshape((self.mesh.num_voxels, self.mesh.dim))
    ...
    def eval_formula(self, formula):
        ...
        for name in field_names:
            ...
            self.formula = self.formula.replace(name, "self.{}".format(escaped_name))
