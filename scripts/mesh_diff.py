...
from pymesh.winding_number import compute_winding_number
import numpy as np
...
def parse_args():
    ...
    parser.add_argument(
        "--num-samples", "-n", type=int, default=5, help="number of samples"
    )
    parser.add_argument(
        "--tol", type=float, default=1e-6, help="winding number tolerance"
    )
    parser.add_argument(
        "--winding-number-engine",
        default="auto",
        choices=["igl", "fast_winding_number"],
    )
    parser.add_argument("--output", "-o", help="Highlighted output", default=None)
    parser.add_argument(
        "--export", "-e", help="export information", action="store_true"
    )
...
def main():
    args = parse_args()
    mesh_1 = mio.load_mesh(args.input_mesh_1)
    mesh_2 = mio.load_mesh(args.input_mesh_2)
    assert mesh_1.dim == 3
    assert mesh_2.dim == 3
    ...
    winding_number_1 = (
        compute_winding_number(mesh_1, queries, engine=args.winding_number_engine) > 0.5
    )
    winding_number_2 = (
        compute_winding_number(mesh_2, queries, engine=args.winding_number_engine) > 0.5
    )

    diff = np.logical_xor(winding_number_1, winding_number_2)
    num_diff = np.count_nonzero(diff)
    print(
        "Winding numbers of {} out of {} samples differ".format(num_diff, len(queries))
    )

    if args.output is not None:
        r = np.amax(bbox_max - bbox_min) * 0.01
        box = mu.generate_box_mesh(np.ones(3) * -r, np.ones(3) * r)
