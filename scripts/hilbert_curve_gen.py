...
def parse_args():
    ...
    parser.add_argument("--resolution", "-r", type=int, help="resolution", default=1)
...
def main():
    ...
    config_transform = np.array(
        [[1, 0, 0, 2], [0, 1, 1, 3], [3, 2, 2, 0], [2, 3, 3, 1]], dtype=int
    )
    ...
    wires_ = wires.WireNetwork.create_from_data(vertices, edges)
    wires_.write_to_file("tmp.obj")
    inflator = wires_.Inflator(wires_)
