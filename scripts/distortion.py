...
from pymesh.predicates import orient_2D
import numpy as np
import numpy.linalg
import logging
import csv
...
def compute_distortion_energies_3D(mesh):
    ...
    regular_tet = mu.generate_regular_tetrahedron()
    assembler = Assembler(regular_tet)
    ...
    invert_J = (
        lambda args: np.full((3, 3), np.inf)
        if args[1] == 0
        else numpy.linalg.inv(args[0])
    )
    ...
    orientations = mu.get_tet_orientations(mesh)
    ...
    num_nonfinite_dirichlet = np.count_nonzero(
        np.logical_not(finite_symmetric_dirichlet)
    )
    ...
    if num_nonfinite_amips > 0:
        logger.warn("Non-finite conformal AMIPS: {}".format(num_nonfinite_amips))
    if num_nonfinite_dirichlet > 0:
        logger.warn(
            "Non-finite symmetric Dirichlet: {}".format(num_nonfinite_dirichlet)
        )
...
def compute_distortion_energies_2D(mesh):
    assert mesh.dim == 2
    regular_tri = mu.generate_equilateral_triangle()
    assembler = Assembler(regular_tri)
    G = assembler.assemble("gradient")
    ...
    orientations = np.array(
        [orient_2D(vertices[t[0]], vertices[t[1]], vertices[t[2]]) for t in tris]
    )
    orientations[orientations > 0] = 1
    orientations[orientations < 0] = -1
