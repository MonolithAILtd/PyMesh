...
def parse_args():
    ...
    parser.add_argument(
        "--time-step", type=float, default=0.001, help="Integration time step"
    )
    parser.add_argument("--num-itrs", type=int, default=1, help="Number of iterations")
    parser.add_argument(
        "--use-graph-laplacian",
        action="store_true",
...
def mean_curvature_flow(mesh, num_itrs=1, time_step=1e-3, use_graph_laplacian=False):
...
def main():
    ...
    mesh = mean_curvature_flow(
        mesh, args.num_itrs, args.time_step, args.use_graph_laplacian
    )
