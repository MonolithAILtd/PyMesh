...
def parse_args():
    ...
    parser.add_argument(
        "--precision", type=int, default=12, help="Number of decimal digits to round to"
    )
...
def main():
    ...
    if args.with_rounding:
        mesh = mio.form_mesh(np.round(mesh.vertices, args.precision), mesh.faces)
    ...
    while len(intersecting_faces) > 0 and counter < args.max_iterations:
        if args.with_rounding:
            ...
            mesh.vertices_ref[involved_vertices, :] = np.round(
                mesh.vertices[involved_vertices, :], args.precision // 2
            )
        mesh = si.resolve_self_intersection(mesh, "igl")
        mesh, __ = mu.remove_duplicated_faces(mesh, fins_only=True)
        if args.with_rounding:
            mesh = mio.form_mesh(np.round(mesh.vertices, args.precision), mesh.faces)
        intersecting_faces = si.detect_self_intersection(mesh)
        counter += 1
