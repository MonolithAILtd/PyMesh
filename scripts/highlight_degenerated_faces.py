...
def parse_args():
    ...
    parser.add_argument(
        "--extract-region", help="exact problematic_region", default=None
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="print degenerated face indices"
    )
...
def main():
    ...
    f_indices = mu.get_degenerated_faces(mesh)
    if len(f_indices) == 0:
    ...
    print(
        "{} degenerated faces, consisting of {} vertices.".format(
    ...
    mio.save_mesh(args.output_mesh, mesh, "degenerated", "degenerated_faces")

    if args.extract_region is not None:
        region = submesh(mesh, f_indices, 0)
        mio.save_mesh(args.extract_region, region, *region.get_attribute_names())
