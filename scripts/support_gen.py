...
def add_base(support_mesh, print_dir, support_length):
    ...
    base_vertices = np.dot(rot.T, base.vertices.T).T + print_dir * np.amin(height)
...
def generate_supports(wire_network, print_dir, support_length, support_thickness, tol):
    ...
    vertices = np.vstack(
        (vertices + 0.05 * print_dir, base_vertices_top_end, base_vertices_bottom_end)
    )
    edges = np.vstack(
        (
            np.arange(num_supports * 2).reshape((num_supports, 2), order="F"),
            np.arange(num_supports, num_supports * 3).reshape(
                (num_supports, 2), order="F"
            ),
        )
    )
    thickness = np.hstack(
...
def parse_args():
    parser = argparse.ArgumentParser(description="Wire mesh support generation")
    parser.add_argument(
        "--print-dir", choices=["X", "Y", "Z", "-X", "-Y", "-Z"], default="Z"
    )
    parser.add_argument(
        "--length", "-l", default=3.0, type=float, help="Support minimum length in mm"
    )
    parser.add_argument(
        "--thickness", "-t", default=0.5, type=float, help="Support thickness in mm"
    )
    parser.add_argument(
        "--support-tol", "-s", type=float, default=1e-6, help="Support tolerance in mm"
    )
...
def main():
    args = parse_args()
    print(
        "Generating support in {} direction of thickness {} and length {}".format(
            args.print_dir, args.thickness, args.length
        )
    )
    ...
    support_wires = generate_supports(
        wire_network, print_dir, args.length, args.thickness, args.support_tol
    )
    support_mesh = inflate_supports(support_wires)
    support_mesh = add_base(support_mesh, print_dir, args.length)
    mio.save_mesh(args.output_mesh, support_mesh)
