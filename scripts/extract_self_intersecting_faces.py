...
def extract_intersecting_faces(mesh, selection):
    face_pairs = si.detect_self_intersection(mesh)
    ...
    intersecting_mesh = mio.form_mesh(mesh.vertices, faces)
    intersecting_mesh, __ = mu.remove_isolated_vertices(intersecting_mesh)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Extract faces intersecting the selection",
    )
...
def main():
    ...
    mio.save_mesh(args.output_mesh, intersecting_mesh)
