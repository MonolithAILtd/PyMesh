...
def triangulate(wires, engine, stage, eps, logger, wire_file, json_file):
    if wires.num_vertices == 0:
        return mio.form_mesh(np.zeros((0, 2)), np.zeros((0, 3)))
    ...
    if engine == "triwild":
        ...
        finish_time = time()
        t = finish_time - start_time
        mesh = mio.load_mesh(out_mesh, drop_zero_dim=True)
    else:
        mesh, t = triangulate_beta(
            wires.vertices, wires.edges, engine=engine, with_timing=True
        )
...
def compute_cell_labels(wires, mesh, logger):
    ...
    cell_ids[cell_type != Arrangement2.ElementType.CELL] = -1
    ...
    if args.with_triangulation:
        if os.path.splitext(args.input_svg)[1] != ".svg" and args.engine == "triwild":
            ...
            with open(wire_file, "w") as fout:
                fout.write(data)
        mesh = triangulate(
            wires, args.engine, args.stage, args.epsilon, logger, wire_file, json_file
        )

        if mesh.num_vertices > 0 and args.with_cell_label:
            compute_cell_labels(wires, mesh, logger)
            mesh = solve_heat_equation(mesh)
            mio.save_mesh(args.output_mesh, mesh, "cell", "sq_dist")
        else:
