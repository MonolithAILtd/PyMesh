...
def parse_args():
    ...
    parser.add_argument("--source", type=int, default=0, help="source vertex index")
...
def assemble_laplacian_matrix(mesh):
    ...
    L = scipy.sparse.coo_matrix(
        (data, (i, j)), shape=(mesh.num_vertices, mesh.num_vertices)
    ).tocsc()
    return L
...
def assemble_mass_matrix(mesh):
    ...
    M = scipy.sparse.dia_matrix(
        (area, (0)), shape=(mesh.num_vertices, mesh.num_vertices)
    )
    return M
...
def compute_gradient(mesh, u):
    ...
    for fi, f in enumerate(mesh.faces):
        ...
        u2 = u[f[2]]
        grad_u = (
            np.cross(N, e0) * u2 + np.cross(N, e1) * u0 + np.cross(N, e2) * u1
        ) / (2 * A)
        grad.append(grad_u)
...
def main():
    ...
    mio.save_mesh(args.output_mesh, mesh, "u", "grad_u", "X", "div_X", "phi")
