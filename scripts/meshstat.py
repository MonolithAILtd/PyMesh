...
from pymesh.predicates import orient_3D
...
def print_basic_info(mesh, info):
    ...
    print("#v: {}\t#f: {}\t#V: {}".format(num_vertices, num_faces, num_voxels))
...
def print_edge_info(mesh, info):
    ...
    quantile_breakdown(
        edge_length, "edge_length", info, title="Edge Length", with_total=False
    )
...
def print_quantile_info(mesh, info):
    ...
    quantile_breakdown(
        vertex_valance, "valance", info, title="Vertex Valance", with_total=False
    )

    mesh.add_attribute("face_aspect_ratio")
    aspect_ratios = mesh.get_attribute("face_aspect_ratio")
    quantile_breakdown(
        aspect_ratios, "aspect_ratio", info, title="Face Aspect Ratio", with_total=False
    )
    ...
    if mesh.num_voxels > 0 and mesh.vertex_per_voxel == 4:
        ...
        quantile_breakdown(
            min_voxel_dihedral_angle,
            "voxel_min_dihedral_angle",
            info,
            title="Per-voxel min dihedral Angle",
            with_total=False,
        )
        quantile_breakdown(
            max_voxel_dihedral_angle,
            "voxel_max_dihedral_angle",
            info,
            title="Per-voxel max dihedral Angle",
            with_total=False,
        )
...
def print_extended_info(mesh, info):
    ...
    unique_vertices = igl_utils.unique_rows(mesh.vertices)[0]
    duplicated_vertices = mesh.num_vertices - len(unique_vertices)

    degenerated_indices = mu.get_degenerated_faces(mesh)
    ...
    if num_degenerated > 0:
        ...
        num_combinatorial_degenerated_faces = len(combinatorially_degenerated_faces)
    else:
    ...
    if num_degenerated > 0:
        print_property(
            "  combinatorially degenerated", num_combinatorial_degenerated_faces, 0
        )
        print_property(
            "  geometrically degenerated",
    ...
    info["num_combinatorial_degenerated_faces"] = num_combinatorial_degenerated_faces
    info["num_geometrical_degenerated_faces"] = (
        num_degenerated - num_combinatorial_degenerated_faces
    )

    if mesh.dim == 2 and mesh.vertex_per_face == 3:
        tri_orientations = mu.get_triangle_orientations(mesh)
    ...
    if mesh.num_voxels > 0 and mesh.vertex_per_voxel == 4:
        tet_orientations = mu.get_tet_orientations(mesh)
...
def coplanar_analysis(mesh, intersecting_faces):
    ...
    for fi, fj in intersecting_faces:
        ...
        if (
            orient_3D(p0, p1, p2, q0) == 0
            and orient_3D(p0, p1, p2, q1) == 0
            and orient_3D(p0, p1, p2, q2) == 0
        ):
...
def print_self_intersection_info(mesh, info):
    if mesh.vertex_per_face == 4:
        ...
        mesh = mu.quad_to_tri(mesh)

    if mesh.num_vertices == 0 or mesh.num_faces == 0:
        num_intersections = 0
        num_coplanar_intersecting_faces = 0
    else:
        intersecting_faces = si.detect_self_intersection(mesh)
    ...
    if num_intersections > 0:
        ...
        print_property(
            "num coplanar intersecting faces", num_coplanar_intersecting_faces, 0
        )
