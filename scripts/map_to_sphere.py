...
from pymesh.tetrahedralize import tetrahedralize
...
def map_boundary_to_sphere(mesh, basename):
    bd_mesh = mio.form_mesh(mesh.vertices, mesh.faces)
    bd_mesh, info = mu.remove_isolated_vertices(bd_mesh)
    bd_vertices = np.copy(bd_mesh.vertices)

    assembler = Assembler(bd_mesh)
    ...
    for i in range(100):
        ...
        if i % 10 == 0:
            sphere_mesh = mio.form_mesh(bd_vertices, bd_mesh.faces)
            mio.save_mesh("{}_flow_{:03}.msh".format(basename, i), sphere_mesh)

    # Then, run mean curvature flow.
    sphere_mesh = mio.form_mesh(bd_vertices, bd_mesh.faces)
    sphere_mesh = mean_curvature_flow(sphere_mesh, 100, use_graph_laplacian=True)
    mio.save_mesh("{}_flow_final.msh".format(basename), sphere_mesh)
    bd_vertices = sphere_mesh.vertices

    # Lastly, project vertices onto unit sphere.
    bd_vertex_indices = info["ori_vertex_index"]
    bd_vertex_positions = np.divide(
        bd_vertices, norm(bd_vertices, axis=1).reshape((-1, 1))
    )
...
def tutte_3D(mesh, bd_vertex_indices, bd_vertex_positions):
    ...
    M, rhs = init_harmonic_system(
        L, bd_vertex_indices, bd_vertex_positions, is_variable
    )
    ...
    return mio.form_mesh(out_vertices, np.zeros((0, 3)), mesh.voxels)
...
def main():
    ...
    mio.save_mesh("{}_source.msh".format(basename), mesh)

    bd_vertex_indices, bd_vertex_positions = map_boundary_to_sphere(mesh, basename)
    out_mesh = tutte_3D(mesh, bd_vertex_indices, bd_vertex_positions)

    tet_orientations = mu.get_tet_orientations(out_mesh)
    out_mesh.add_attribute("orientation")
    out_mesh.set_attribute("orientation", tet_orientations)
    mio.save_mesh(args.output_mesh, out_mesh, "orientation")
