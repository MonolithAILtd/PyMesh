...
void Manifold::run(int depth) {
  BuildTree(depth);
  ConstructManifold();

  MV_ = MatrixFr(vertices_.size(), 3);
  MF_ = MatrixIr(face_indices_.size(), 3);
  for (int i = 0; i < vertices_.size(); ++i)
    MV_.row(i) = vertices_[i];
  for (int i = 0; i < face_indices_.size(); ++i)
    MF_.row(i) = face_indices_[i];

  MeshProjector projector;
  projector.Project(V_, F_, MV_, MF_);
}
...
void Manifold::CalcBoundingBox() {
  min_corner_ = Vector3F(1e30, 1e30, 1e30);
  ...
  Vector3F volume_size = max_corner_ - min_corner_;
  for (int i = 0; i < 3; ++i) {
  ...
  double weird_number[3][2] = {
      {0.1953725, 0.1947674}, {0.1975733, 0.1936563}, {0.1957376, 0.1958437}};
  for (int i = 0; i < 3; ++i) {
...
void Manifold::ConstructManifold() {
  ...
  tree_->ConstructFace(Vector3I(0, 0, 0), &vcolor, &nvertices, &nface_indices,
                       &v_faces);
...
bool Manifold::SplitGrid(const std::vector<Vector4I> &nface_indices,
                         std::map<GridIndex, int> &vcolor,
                         std::vector<Vector3F> &nvertices,
                         std::vector<std::set<int>> &v_faces,
                         std::vector<Vector3I> &triangles) {
  ...
  for (auto it = vcolor.begin(); it != vcolor.end(); ++it) {
  ...
  for (auto it = edge_info.begin(); it != edge_info.end(); ++it) {
    if (it->second.size() > 2) {
  ...
  triangles.clear();
  Float half_len =
      (nvertices[nface_indices[0][1]] - nvertices[nface_indices[0][0]]).norm() *
      0.5;
  for (int i = 0; i < (int)nface_indices.size(); ++i) {
    int t = 0;
    while (t < 4 && marked_v.find(nface_indices[i][t]) == marked_v.end())
      ++t;
    if (t == 4) {
      triangles.push_back(Vector3I(nface_indices[i][0], nface_indices[i][2],
                                   nface_indices[i][1]));
      triangles.push_back(Vector3I(nface_indices[i][0], nface_indices[i][3],
                                   nface_indices[i][2]));
      continue;
    }
    int ind[4];
  ...
  for (auto it = marked_v.begin(); it != marked_v.end(); ++it) {
    Vector3F p = nvertices[*it];
    for (int dimx = -1; dimx < 2; dimx += 2) {
      for (int dimy = -1; dimy < 2; dimy += 2) {
        for (int dimz = -1; dimz < 2; dimz += 2) {
          Vector3F p1 =
              p + Vector3F(dimx * half_len, dimy * half_len, dimz * half_len);
          if (tree_->IsExterior(p1)) {
            ...
            if (vcolor.find(ind1) == vcolor.end()) {
              ...
              nvertices.push_back(Vector3F(p[0] + half_len * dimx, p[1], p[2]));
            ...
            if (vcolor.find(ind2) == vcolor.end()) {
              ...
              nvertices.push_back(Vector3F(p[0], p[1] + half_len * dimy, p[2]));
              v_faces.push_back(v_faces[*it]);
            }
            if (vcolor.find(ind3) == vcolor.end()) {
              ...
              nvertices.push_back(Vector3F(p[0], p[1], p[2] + half_len * dimz));
              v_faces.push_back(v_faces[*it]);
            }
            int id1 = vcolor[ind1];
            int id2 = vcolor[ind2];
            int id3 = vcolor[ind3];
            Vector3F norm = (nvertices[id2] - nvertices[id1])
                                .cross(nvertices[id3] - nvertices[id1]);
            if (norm.dot(Vector3F(dimx, dimy, dimz)) < 0)
  ...
  for (auto it = evens.begin(); it != evens.end(); ++it) {
    ...
    int count = 0;
    for (int j = 0; j < 8; ++j) {
      Vector3F d((j & 0x04) > 0, (j & 0x02) > 0, (j & 0x01) > 0);
      d = d * 2.0 - Vector3F(1, 1, 1);
  ...
  for (auto it = odds.begin(); it != odds.end(); ++it) {
    ...
    if (edge_info[std::make_pair(x, y)].size() > 2) {
      Vector3F vert = nvertices[x] - nvertices[y];
      Float len = vert.norm();
      vert /= len;
      Vector3F dir(len * 0.5, len * 0.5, len * 0.5);
