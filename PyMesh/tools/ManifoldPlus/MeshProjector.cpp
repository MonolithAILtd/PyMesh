  ...
  for (int i = 0; i < num_V_; ++i) {
    ...
    if (vertex_count != vlinks[i].size()) {
      ...
      while (!vlinks[i].empty()) {
        ...
        if (group_id != 0) {
          insert_vertex_info.push_back(std::make_pair(num_vertices, i));
          for (auto &p : dedges) {
...
void MeshProjector::Project(const MatrixFr &V, const MatrixIr &F,
                            MatrixFr &out_V, MatrixIr &out_F) {
  V_ = V;
  F_ = F;
  out_V_ = out_V;
  out_F_ = out_F;

  Float len = (out_V_.row(out_F_(0, 0)) - out_V_.row(out_F_(0, 1))).norm();
...
void MeshProjector::UpdateFaceNormal(int i) {
  ...
  do {
    ...
    Vector3F d0 = out_V_.row(v1) - out_V_.row(v0);
    Vector3F d1 = out_V_.row(v2) - out_V_.row(v0);
    d0.normalize();
    d1.normalize();
    auto vn = d0.cross(d1);
    double l = vn.norm();
...
int MeshProjector::BoundaryCheck() {
  ...
  int inconsistent = 0;
  for (int i = 0; i < num_V_; ++i) {
    Vector3F n = out_N_.row(i);
    int deid = V2E_[i];
    int deid0 = deid;
    do {
      Vector3F fn = out_FN_.row(deid / 3);
      if (n.dot(fn) < -ZERO_THRES) {
        inconsistent += 1;
        printf("%d %d %f: <%f %f %f> <%f %f %f>\n", i, deid / 3, n.dot(fn),
               n[0], n[1], n[2], fn[0], fn[1], fn[2]);
      } else
        consistent += 1;
      deid = E2E_[deid / 3 * 3 + (deid + 2) % 3];
    } while (deid0 != deid);
...
void MeshProjector::IterativeOptimize(Float len, bool initialized) {
    ...
    for (int i = 0; i < sharp_vertices_.size(); ++i) {
            if (sharp_vertices_[i] > 0) {
                    sqrD_[i] = 10000 * sharp_vertices_[i] + (sharp_positions_[i]
                            - Vector3F(out_V_.row(i))).squaredNorm();
...
// void MeshProjector::Highlight(int id, Float len) {
//   UpdateNearestDistance();
//   double max_dis = 0;
//   std::vector<Float> distances(sqrD_.size());
//   memcpy(distances.data(), sqrD_.data(), sizeof(Float) * sqrD_.size());
...
//     if (dis > max_dis - 1e-7) {
//       printf("Id %d\n", i);
//       Vector3F d1 = target_V_.row(i) - out_V_.row(i);
//       printf("Distance0 %lf\n", d1.norm());
//       OptimizePosition(i, target_V_.row(i), len, true);
//       Vector3F d2 = target_V_.row(i) - out_V_.row(i);
//       printf("Distance1 %lf\n", d2.norm());
//       Vector3F v = out_V_.row(i);
//       Vector3F n = out_N_.row(i);
//       os << "v " << v[0] << " " << v[1] << " " << v[2] << " 0 0.99 0\n";
//       v += n * 1e-3;
//       os << "v " << v[0] << " " << v[1] << " " << v[2] << " 0.99 0 0\n";
//       Vector3F p = target_V_.row(i);
...
void MeshProjector::AdaptiveRefine(Float len, Float ratio) {
  ...
  auto AddVertex = [&](const Vector3F &p, const Vector3F &n,
                       const Vector3F &tar_p, const Float &sqr_dis,
                       int face_index, int sharp) {
  ...
  auto AddFace = [&](const Vector3I &f, const Vector3F &n) {
  ...
  for (int iter = 0; iter < 2; ++iter) {
    ...
    for (int i = 0; i < candidates.size(); ++i) {
      ...
      Vector3F p0 = V_.row(F_(src_f0, 0));
      Vector3F n0 = origin_FN.row(src_f0);

      Vector3F p1 = V_.row(F_(src_f1, 0));
      Vector3F n1 = origin_FN.row(src_f1);
      ...
      Vector3F np = P.row(i);
      Vector3F ntarget1 = (np - o).dot(t) * t + o;
    ...
    for (auto &info : face_splits) {
      ...
      for (int i = 0; i < 3; ++i) {
        int v0 = f[i];
        Float dis = (out_V_.row(v0) - target_V_.row(v0)).norm();
  ...
  while (true) {
    ...
    for (int i = 0; i < candidates.size(); ++i) {
      ...
      if ((out_V_.row(v0) - out_V_.row(v1)).squaredNorm() >
        ...
        int f1 = e1 / 3;
        int f2 = e2 / 3;
        Vector3F d1 = out_V_.row(v3) - out_V_.row(v2);
        Vector3F d2 = out_V_.row(v0) - out_V_.row(v2);
        Vector3F d3 = out_V_.row(v1) - out_V_.row(v2);
        Vector3F n1 = d2.cross(d1);
        Vector3F n2 = d1.cross(d3);
        if (n1.norm() > 0)
          n1 = n1 / n1.norm();
        if (n2.norm() > 0)
          n2 = n2 / n2.norm();
...
void MeshProjector::OptimizePosition(int v, const Vector3F &p, Float len,
                                     bool debug) {
  ...
  do {
    ...
    Vector3F vn[] = {out_N_.row(v0), out_N_.row(v1), out_N_.row(v2)};
    for (int i = 0; i < 3; ++i) {
      Vector3F d = (out_V_.row(v2) - out_V_.row(v1));
      d = d.cross(vn[i]).normalized();
      Float b = d.dot(out_V_.row(v1) - out_V_.row(v0));
  ...
  std::vector<Vector3F> constraints;
  constraints.reserve(3);

  for (int i = 0; i < A.size(); ++i) {
    Vector3F offset = p - Vector3F(out_V_.row(v));

    Float tar_step = offset.norm();
    if (tar_step < ZERO_THRES)
      return;

    Vector3F tar_dir = offset / tar_step;

#ifdef PROJ_THREE_TIMES
    if (constraints.size() == 1) {
      tar_dir = tar_dir - tar_dir.dot(constraints[0]) * constraints[0];
      Float n = tar_dir.norm();
      if (n < ZERO_THRES)
        return;
      tar_step *= n;
      tar_dir /= n;
    } else if (constraints.size() == 2) {
      Vector3F dir = constraints[0].cross(constraints[1]).normalized();
    ...
    if (constraints.size() > 0) {
      Vector3F c = constraints.back();
      Vector3F temp_dir = tar_dir - tar_dir.dot(c) * c;
      Float n = temp_dir.norm();
      if (n < ZERO_THRES)
        return;
      temp_dir /= n;
      int boundary_constraint = 0;
      Vector3F temp_boundary[3];
      for (int j = 0; j < constraints.size(); ++j) {
        Float denominator = constraints[j].dot(temp_dir);
        if (denominator > -1e-3) {
      ...
      if (top == 2) {
        Vector3F c1 = constraints[0];
        Vector3F c2 = constraints[1];
        Vector3F dir = c1.cross(c2).normalized();
    ...
    Float max_step = tar_step;
    int constraint_id = -1;
    for (int j = 0; j < A.size(); ++j) {
      if (attached_dimensions[j])
        continue;
      Float denominator = A[j].dot(tar_dir);
      if (denominator < ZERO_THRES)
        continue;
      Float step = B[j] / denominator;
      if (step < max_step) {
        constraint_id = j;
        max_step = step;
      }
    ...
    for (int j = 0; j < A.size(); ++j) {
      ...
      if (B[j] < ZERO_THRES && denominator >= ZERO_THRES) {
        ...
        if (constraint_size == 2) {
          Vector3F n = constraints[0].cross(constraints[1]);
...
void MeshProjector::OptimizeNormal(int i, const Vector3F &vn,
                                   const Vector3F &target_vn) {
  Vector3F d = target_vn - vn;
  Float max_step = 1.0;
  int deid = V2E_[i];
  int deid0 = deid;
  do {
    Vector3F fn = out_FN_.row(deid / 3);
    Float denominator = d.dot(fn);
    if (denominator < -ZERO_THRES) {
      Float step = -fn.dot(vn) / denominator;
      if (step < max_step) {
        max_step = step;
      }
    }
...
void MeshProjector::OptimizeNormals() {
  MatrixFr prev_norm = out_N_;
  UpdateVertexNormals(0);
  igl::per_face_normals(out_V_, out_F_, out_FN_);
  for (int i = 0; i < num_V_; ++i) {
    Vector3F vn = prev_norm.row(i);
    Vector3F target_vn = out_N_.row(i);
    OptimizeNormal(i, vn, target_vn);
  }
}

void MeshProjector::PreserveSharpFeatures(Float len_thres) {
  /*
  UpdateNearestDistance();
  MatrixFr origin_FN;
  igl::per_face_normals(V_, F_, origin_FN);
  ...
  auto consistent = [&](int src_f0, int src_f1) {
          if (src_f0 == src_f1)
                  return true;
          Vector3F n1 = origin_FN.row(src_f0);
          Vector3F n2 = origin_FN.row(src_f1);
          Float norm_angle = std::abs(n1.dot(n2));
  ...
  for (int i = 0; i < num_F_; ++i) {
          for (int j = 0; j < 3; ++j) {
                  ...
                  Vector3F u1 = out_V_.row(v0);
                  Vector3F u2 = out_V_.row(v1);
                  Vector3F ntarget1 = (u1 - o).dot(t) * t + o;
                  Vector3F ntarget2 = (u2 - o).dot(t) * t + o;
  ...
  MatrixFr P(vertex_positions.size(), 3);
  memcpy(P.data(), vertex_positions.data(),
          sizeof(Vector3F) * vertex_positions.size());
  VectorF sqrD;
  VectorI I;
  MatrixFr tarP;
  */
  ...
  std::vector<Vector3F> vertex_update_position;
  ...
  for (auto &v : vfeatures) {
    ...
    if (v.size() > 0) {
      ...
      Vector3F p0 = V_.row(F_(face_elements[0].first, 0));
      Vector3F n0 = origin_FN.row(face_elements[0].first);

      Vector3F p1 = V_.row(F_(face_elements[0].second, 0));
      Vector3F n1 = origin_FN.row(face_elements[0].second);

      Vector3F o, t;
      if (!PlaneIntersect(p0, n0, p1, n1, &o, &t)) {
        continue;
      }
      Vector3F target = out_V_.row(vid), ntarget;
      bool solved = false;
      ...
      if (face_elements.size() > 1) {
        ...
        Float max_len = 1e30;
        for (auto &p : fset) {
          Vector3F p2 = V_.row(F_(p, 0));
          Vector3F n2 = origin_FN.row(p);
          if (std::abs(t.dot(n2)) > 0.1) {
            Float lambda = (p2 - o).dot(n2) / t.dot(n2);
            Vector3F nt = o + lambda * t;
            Float len = (nt - target).norm();
            if (len < max_len) {
              max_len = len;
              ntarget = nt;
            }
      ...
      if (!solved) {
        ntarget = (Vector3F(target) - o).dot(t) * t + o;
      }
  ...
  MatrixFr P(vertex_update_position.size(), 3);
  memcpy(P.data(), vertex_update_position.data(),
         sizeof(Vector3F) * vertex_update_position.size());
  VectorF sqrD;
  VectorI I;
  MatrixFr tarP;
  sharp_vertices_.resize(num_V_, 0);
  ...
  for (int i = 0; i < sqrD.size(); i += 2) {
          ...
          if (sqrt(sqrD[i]) < len_thres * 1e-1) {
                  ...
                  os << "v " << vertex_positions[i][0] << " " <<
  vertex_positions[i][1] << " " << vertex_positions[i][2] << "\n";
          }
          if (sqrt(sqrD[i + 1]) < len_thres * 1e-1) {
                  sharp_vertices_[v1] = 1;
                  sharp_positions_[v1] = vertex_positions[i + 1];
                  os << "v " << vertex_positions[i+1][0] << " " <<
  vertex_positions[i+1][1] << " " << vertex_positions[i+1][2] << "\n";
          }
  }
  os.close();
  */
