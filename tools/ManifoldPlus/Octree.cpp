...
Octree::Octree(const Vector3F min_c, const Vector3F max_c,
  ...
  volume_size_ = Vector3F(1, 1, 1) * volume_size_[ind];
...
Octree::Octree(const Vector3F &min_c, const Vector3F &volume_size) {
  memset(children_, 0, sizeof(Octree *) * 8);
  memset(connection_, 0, sizeof(Octree *) * 6);
  memset(empty_connection_, 0, sizeof(Octree *) * 6);
  level_ = 0;
...
bool Octree::IsExterior(const Vector3F &p) {
  for (int i = 0; i < 3; ++i)
...
bool Octree::Intersection(int Find_ex, const Vector3F &min_corner,
...
void Octree::Split(const MatrixFr &V) {
  level_ += 1;
  number_ = 0;
  if (level_ > 1) {
    for (int i = 0; i < 2; ++i) {
  ...
  Vector3F halfsize = volume_size_ * 0.5;
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 2; ++j) {
      for (int k = 0; k < 2; ++k) {
        int ind = i * 4 + j * 2 + k;

        Vector3F startpoint = min_corner_;
  ...
  if (dim == 2) {
    ...
    for (int i = 0; i < 4; ++i) {
      if (l->children_[y_index[i] + 2] && r->children_[y_index[i]]) {
        ConnectTree(l->children_[y_index[i] + 2], r->children_[y_index[i]],
                    dim);
      }
    }
  } else if (dim == 0) {
...
void Octree::ConnectEmptyTree(Octree *l, Octree *r, int dim) {
  ...
  if (l->occupied_ && r->occupied_) {
    ...
    if (dim == 2) {
      for (int i = 0; i < 4; ++i) {
        ConnectEmptyTree(l->children_[i * 2 + 1], r->children_[i * 2], dim);
      }
    } else if (dim == 1) {
      for (int i = 0; i < 4; ++i) {
        ConnectEmptyTree(l->children_[y_index[i] + 2], r->children_[y_index[i]],
                         dim);
      }
    } else if (dim == 0) {
      for (int i = 0; i < 4; ++i) {
...
void Octree::ExpandEmpty(std::list<Octree *> &empty_list,
  ...
  if (dim == 2 || dim == 5) {
    for (int i = 0; i < 4; ++i) {
      children_[i * 2 + (dim == 5)]->ExpandEmpty(empty_list, empty_set, dim);
    }
    return;
  }
  if (dim == 1 || dim == 4) {
    for (int i = 0; i < 4; ++i) {
      children_[y_index[i] + 2 * (dim == 4)]->ExpandEmpty(empty_list, empty_set,
                                                          dim);
    }
    return;
  }
  for (int i = 0; i < 4; ++i) {
    children_[i + 4 * (dim == 3)]->ExpandEmpty(empty_list, empty_set, dim);
  }
}
...
void Octree::ConstructFace(const Vector3I &start,
  ...
  if (level_ == 0) {
    if (!occupied_)
      return;
    Vector3I offset[6][4] = {{Vector3I(1, 0, 0), Vector3I(1, 0, 1),
    ...
    for (int i = 0; i < 8; ++i) {
      if (children_[i] && children_[i]->occupied_) {
        ...
        children_[i]->ConstructFace(nstart, vcolor, vertices, faces, v_faces);
      }
    }
  }
}
