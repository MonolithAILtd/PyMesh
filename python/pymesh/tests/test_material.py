    ...
    class MaterialTest(TestCase):
        ...
        def test_isotropic_material(self):
            ...
            self.assertAlmostEqual(
                young * (1 - poisson) / (1 + poisson) / (1 - 2 * poisson), stress[0, 0]
            )
            self.assertAlmostEqual(
                young * poisson / (1 + poisson) / (1 - 2 * poisson), stress[1, 1]
            )
            self.assertAlmostEqual(
                young * poisson / (1 + poisson) / (1 - 2 * poisson), stress[2, 2]
            )
        ...
        def test_isotropic_material_2D(self):
            ...
            stress = mat.strain_to_stress(strain)
            self.assertAlmostEqual(
                strain[0, 0], stress[0, 0] / young - poisson * stress[1, 1] / young
            )
            self.assertAlmostEqual(
                strain[1, 1], stress[1, 1] / young - poisson * stress[0, 0] / young
            )
            self.assertAlmostEqual(strain[0, 1], stress[0, 1] * (1 + poisson) / young)
        ...
        def test_equivalence(self):
            ...
            ortho_mat = Material.create_orthotropic(
                1.0,
                np.ones(3) * young,
                np.ones(6) * poisson,
            ...
            self.assert_array_almost_equal(
                iso_mat.material_tensor, ortho_mat.material_tensor
            )
