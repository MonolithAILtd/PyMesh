...
def generate_tube(
    ...
    vertices = np.vstack(
        [bottom_outer_rim, bottom_inner_rim, top_outer_rim, top_inner_rim]
    )
    ...
    if with_quad:
        ...
        bottom = np.array([[(i + 1) % N, i, N + i, N + (i + 1) % N] for i in range(N)])
        inner = np.array(
            [[3 * N + i, 3 * N + (i + 1) % N, N + (i + 1) % N, N + i] for i in range(N)]
        )
        outer = np.array(
            [[i, (i + 1) % N, 2 * N + (i + 1) % N, 2 * N + i] for i in range(N)]
        )
