...
def generate_box_mesh(
    ...
    dim = len(box_min)
    if dim == 2:
        mesh, cell_index = generate_2D_box_mesh(
            box_min, box_max, num_samples, keep_symmetry, subdiv_order, using_simplex
        )
    elif dim == 3:
        mesh, cell_index = generate_3D_box_mesh(
            box_min, box_max, num_samples, keep_symmetry, subdiv_order, using_simplex
        )
    ...
    for i in range(num_samples[0]):
        for j in range(num_samples[1]):
            ...
            for corners in subcell_corners:
                if using_simplex:
                    if keep_symmetry:
                        cell_vertices, cell_faces = split_quad_into_tris_symmetrically(
                            corners
                        )
                    else:
                        cell_vertices, cell_faces = split_quad_into_tris(corners)
                else:
...
def subdivide_hex(corners, order):
    ...
    if order == 0:
        ...
        face_centers = np.array([np.mean(corners[face], axis=0) for face in faces])
