...
class Inflator(object):
    ...
    def set_refinement(self, order=1, method="loop"):
        ...
        if method not in ("loop", "simple"):
            raise NotImplementedError("Unsupport subdivision method: {}".format(method))

        self.subdivide_order = order
        self.subdivide_method = method

    def inflate(
        self, thickness, per_vertex_thickness=True, allow_self_intersection=False
    ):
    ...
    def __setup_geometry_correction(self, inflator):
        ...
        if self.geometry_correction_lookup is not None:
            inflator.with_geometry_correction_lookup(self.geometry_correction_lookup)

    def __setup_subdivision(self, inflator):
        inflator.with_refinement(self.subdivide_method, self.subdivide_order)
    ...
    def mesh(self):
        ...
        if self.wire_network.has_attribute("vertex_symmetry_orbit"):
            ...
            source_index[source_id_mask] = indices[
                self.source_wire_id[source_id_mask] - 1
            ]
        ...
        if self.wire_network.has_attribute("vertex_cubic_symmetry_orbit"):
            indices = self.wire_network.get_attribute(
